...
public class UserService {
    ...
    public void insertCartItem(List<CartItems> cartItems, HttpSession session) throws SQLException {
        String query = "INSERT INTO orders (userName, name, price, quantity, productId, orderDate) VALUES (?, ?, ?, ?, ?, ?)";
        PreparedStatement preparedStatement = new DBConnection().getStatement(query);
        try {
            for(CartItems cartItem : cartItems) {
                preparedStatement.setString(1, (String)session.getAttribute("userName"));
                preparedStatement.setString(2, cartItem.getName());
                ...
                preparedStatement.setDate(6, new java.sql.Date(cartItem.getOrderDate().getTime()));
    ...
    public void insertIntoCart(Product product, HttpSession session) throws SQLException {
        String query = "INSERT INTO cart (userName, email, name, price, quantity, discount, image) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE quantity=quantity+1";
        PreparedStatement preparedStatement = new DBConnection().getStatement(query);
        try {
